    ...
    onMouseDrag:function(canvas, dx, dy, dx2, dy2, shiftKey, ctrlKey)
    {
    	if( !((this.mouseDraggingElement instanceof draw2d.ResizeHandle) || (this.mouseDraggingElement instanceof draw2d.Port))){
	    	if(this.cloneOnDrag ===true && this.mouseDraggingElement !==null){
                ...
                //FIX CLONE HACK
                ele = app.view.getSelection().all.data[0];
                if(ele.cssClass=='ModuleShape'){
                ports = ele.getPorts().data;
                ports.forEach(function (port,idx){
                    port.connectionAnchor.owner.name='input_'+ele.getEntity(idx).id;
                });
                }	
                if(ele.cssClass=='TableShape'){
                    ports = ele.getPorts().data;
                    ports.forEach(function (port,idx){
                        if(port.cssClass=='draw2d_InputPort'){str = 'input_'}else{str='output_'}
                        port.connectionAnchor.owner.name=str+ele.getEntity(Math.round((idx+1)/2)-1).id;
                    });
                    }
                if(ele.cssClass=='filePathShape' || ele.cssClass=='outputShape' || ele.cssClass =='scriptShape'){
                    ports = ele.getPorts().data;
                    ports.forEach(function (port,idx){
                        if(port.cssClass=='draw2d_InputPort'){str = 'input_'}else{str='output_'}
                        port.connectionAnchor.owner.name=str+ele.id;
                    });
                    }		
